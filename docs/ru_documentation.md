...
Запросы могут быть только текстовыми, так как сама языковая модель текстовая. Она не может обрабатывать документы или какие-либо изображения, поэтому бот не отвечает на такие запросы. То же самое применяется и к голосовым сообщениям с кружочками.
...
1. **Максимальная длина ответа бота - 4000 символов. Если его ответ содержит большее количество символов, то он его 'обрежет' до 4000 сиволов.**
...
2. **"🤷 Не получилось сформировать ответ. Попробуйте задать вопрос иначе."** - Может произойти по причине, если DeepSeek вернул пустой ответ. Это может произойти из-за перенагрузки сервера DeepSeek, либо вы задали вопрос неккоректно. В таком случае попробуйте задать вопрос иначе.
3. **"⚠️ Не удалось отправить ответ. Пожалуйста, попробуйте еще раз."** - Эта ошибка может возникнуть, если сам бот не смог отправить сообщение. В этом случае попробуйте задать запрос по-другому или задать тот же запрос еще раз.
...
3. Бот:
    ...
    * Выполняет запрос в Postgres (добавление сообщения, для его сохранения в истории разговора и контекста переписки)
    * Отправляет текст DeepSeek API
...
func (h *NeuralHandler) HandleMessage(ctx context.Context, userID int64, text string) (string, error) { 
    // 1. Сохранение запроса пользователя
    // 2. Получение сообщений для понимания контекста диалога
    // 3. Генерирует и направляет запрос
...
base-url: "<DEEPSEEK URL>" # (без "/chat/completions")
...
1. **Бот хранит персональные данные пользователей для и отличия. Не волнуйтесь, единственное, что относится к персональным данным для хранения - Телеграм-айди, который бот использует для отличия пользователей друг от друга. Это используется для сохранения контекста диалога с нейросетью:**
```
...
1. **Телеграм-айди пользователя.**
2. **Запрос, который пользователь отправил нейросети.**
3. **Дату отправки запроса.**
4. **Помимо ваших запросов, база данных также хранит ответы нейросети.***

**Такие данные в базе данных автоматически сбрасыватся ежедневно.**
